...
public class MasterMusic {
	...
	public TextureAtlas getTextureAtlas(Array<String> soundHeader){
		for(int i = 0; i < soundHeader.size; i++){
			if(soundHeader.get(i) == "TEXTURE_ATLAS_LOCATION"){
				return new TextureAtlas(Gdx.files.external(header.get(i + 1)));
			}
		}
		return null;
	}
	public HashMap<Object, Integer> loadSounds(Array<String> soundHeader, TextureAtlas texture_atlas){
